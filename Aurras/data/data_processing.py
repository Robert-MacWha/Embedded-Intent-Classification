...
def preprocess_intent_dataset(samples_per_intent: int, data_path: str):
	"""
		...
		Outputs:
		 - raw_sorted_data: {"intent_name": ["prompt", "prompt", ...]}
	"""
...
def process_triplets(raw_sorted_data: json):
	""" 
		...
		Outputs:
		 - triplet_data: array of triplet tuples
	"""
...
def tokenize_triplet_intent_dataset(triplet_data: list, tokenizer: object, padding: int):
	"""
		Tokenizes the triplet text data & return it

		Inputs:
		 - triplet_data: list of tuples containing (anchor, positive, and negative) strings
		 - tokenizer: huggingface tokenizer
		 - padding: maximum length of the output arrays
		
		Outputs:
		 - tokenized_data: list of triplet tokenized data: (anchor_ids, anchor_attention, positive_ids, positive_attention, negative_ids, negative_attention)
	"""
